...
<table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered col-md-12 col-xs-12">
    ...
    <tbody><?php 
        		...
        		$query = mysql_query("SELECT * FROM employee_db WHERE position = 4");
        		while($keps = mysql_fetch_array($query)){
        	 ?>
        <tr class="gradeX">
        	
            <td><center><?php echo $keps['full_name']; ?></center></td>
            <td><center><?php echo $keps['department']; ?></center></td>
            <td><center><?php echo $keps['id']; ?></center></td>
            <td><center>
            	<?php if(md5($keps['id']) != $eid){ ?>
            	<a class="btn btn-danger" href="/abs/process/delete_employee.php?eid=<?php echo $eid; ?>&emp_num=<?php echo $keps['id']; ?>">Fire Employee</a>
            	<?php } ?>
            </center></td>
